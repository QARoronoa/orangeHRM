...
def user_clique_sur_signup(setup):
    with allure.step("Clique sur le bouton 'Sign up' (header)"):
        home_page = HomePage(setup)
...
def user_visualise_la_popin_signup(setup):
    with allure.step('Vérifie que la popin de signup'):
        home_page = HomePage(setup)
...
def user_clique_sur_signup_boutton_popin(setup):
    with allure.step('Clique sur le bouton "Signup" dans la popin'):
        home_page = HomePage(setup)
